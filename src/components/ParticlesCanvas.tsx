  ...
  useEffect(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      particles.push({
        ...
        rotationSpeed: (Math.random() - 0.5) * 0.02, // velocidad de rotación muy leve
      });
    }
    ...
    const animate = () => {
      ...
      particles.forEach((p, idx) => {
        ...
        // Dibujar cuadrado centrado en el origen
        ctx.beginPath();
...
//   p.radius * 2, 
//   p.radius * 2, 
//   p.radius * 0.5  // 👈 Radio de las esquinas redondeadas (ajústalo a tu gusto)
// );
// ctx.fill();
  ...
  return (
    <canvas 
      ref={canvasRef} 
      className="absolute inset-0 z-0 h-auto w-auto" 
      style={{ display: 'block' }}
    />
  );
};
