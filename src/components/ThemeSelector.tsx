...
export default function ThemeSelector() {
  ...
  const themes = [
    ...
    { value: "light" as const, label: "Light", icon: <TbBulbFilled size={20} /> },
    { value: "vibrant" as const, label: "Vibrant", icon: <RiLightbulbFlashFill size={20} /> },
  ];
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full mt-2 flex flex-col shadow-lg z-10 rounded bg-theme-secondary">
          {themes.map((t) => (
            ...
            >
              {t.icon}
              <span className="absolute left-full ml-2 px-2 py-1 text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap pointer-events-none bg-[var(--bg-tertiary)]">
                {t.label}
              </span>
            </button>
          ))}
