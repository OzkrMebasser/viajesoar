    ...
    <Step number={number} label={t(locale, "Comentarios y consentimiento", "Comments & consent")}>
      <Field icon={<FaCommentDots />} label={t(locale, "Comentarios", "Comments")}>
        <textarea
          ...
          placeholder={t(locale, "¿Alguna solicitud especial?", "Any special requests?")}
      ...
      <div className="space-y-2 pt-1">
        <label className="flex items-start gap-2 cursor-pointer group">
          ...
          <span className="text-[var(--text)]/50 text-xs leading-relaxed group-hover:text-[var(--text)]/70 transition-colors">
            {t(locale, "Acepto términos y condiciones", "I accept terms and conditions")}
            <span className="text-[var(--accent)]"> *</span>
          </span>
        </label>
        <label className="flex items-start gap-2 cursor-pointer group">
          <input
            type="checkbox" name="newsletter" checked={form.newsletter}
            onChange={onChange}
            className="mt-0.5 accent-[var(--accent)]"
          />
          <span className="text-[var(--text)]/50 text-xs leading-relaxed group-hover:text-[var(--text)]/70 transition-colors">
            {t(locale, "Quiero recibir ofertas en mi email", "I want to receive offers by email")}
          </span>
        </label>
      </div>
    </Step>
