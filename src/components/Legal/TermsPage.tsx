...
import termsData from "../../lib/data/terms/terms.json"; 

interface Props {
  locale: Locale;
}

const heroImgEN = "https://res.cloudinary.com/dtsenvmdq/image/upload/v1780616541/terms-and-conditions_umpcwr.png";
const heroImgES = "https://res.cloudinary.com/dtsenvmdq/image/upload/v1780616585/terminos-y-condiciones-de-servicio_n5mhmd.png";

export default function TermsPage({ locale }: Props) {
  const { meta, sections, contact } = termsData;
 const currentHeroImg = locale === "es" ? heroImgES : heroImgEN;
  return (
    <section className="min-h-screen bg-gradient-theme">
      ...
      <div className="absolute inset-0 z-10 flex flex-col justify-center px-4 sm:px-6 pb-0 lg:pb-8 text-white">
        <div className="flex items-center gap-2 mb-4">
          <FileText className="text-[var(--accent)] w-4 h-4" />
      ...
      {/* Scroll indicator */}
      <ScrollIndicator targetId="terms-content" />
