...
/* ── Related Post Card ── */
function RelatedCard({ post, locale }: { post: BlogPost; locale: Locale }) {
    ...
    <article className="glass-card border border-white/10 rounded-sm overflow-hidden hover:border-[var(--accent)]/30 transition-all duration-300 flex flex-col relative">
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="mt-auto">
          <ButtonArrow href={href} title={t(locale, "Leer", "Read")} />
        </div>
      </div>
    </article>
  );
...
export default function BlogArticle({ locale, post, relatedPosts }: Props) {
  ...
  const galleryImages = [post.cover_image, ...(post.photos ?? [])].filter(
    Boolean,
  ) as string[];
    ...
    <div className="min-h-screen bg-gradient-theme">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="mb-6 flex flex-wrap gap-3 items-center">
          ...
          gap-1.5 text-[var(--text)]/50 hover:text-sky-400 transition-colors
          text-sm"
          <a>
            {" "}
            <FaTwitter /> Twitter
